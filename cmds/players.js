    ...
    try {
      ...
      req(api2, function (err, response, main) {
        req(api, function (err, response, body) {
          if (err) {
            util.zembed("Esse servidor não existe ou está OFF... \n**Console:**\n```js\n" + err + "```")
          }
          else {
            try {
              ...
              var embed = new Discord.RichEmbed()
                ...
                .setThumbnail(icon)
                .setDescription(`**${e}** de **${start2.vars.sv_maxClients}** Players.\n\n*(Primeiras 25 Pessoas)*\n\n${news}`)
		.setFooter(`Server IP: ${servers[message.guild.id].ip}`, icon)
              ...
              util.embed("Esse servidor não existe ou está OFF...\n**Console:**\n```js\n" + err + "```")
            }
          }
