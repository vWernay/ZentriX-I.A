...
exports.run = (client, message, args) => {
    ...
    let argsJunto = message.content.split(" ").slice(1).join(' ')
        ...
        if (message.guild === null) {
            message.channel.sendMessage('<:vpRedTick:695064695488643093> | Você não parece estar executando o comando num servidor.');
        } else {
    ...
    membrosOn.push(member.user.username)
            ...
            if (message.channel.permissionsFor(message.guild.member(client.user)).hasPermission('EMBED_LINKS')) {
